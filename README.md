...
- `endpoints[]`: list of `{host, user, path}`. Each endpoint may also carry:
  ...
  - `log_path`: path on the **worker** where `gorn wrap` appends its diagnostic lines and the running command's stdout/stderr. Command output is emitted as JSONL records carrying `guid`, `root`, `stream`, and `line`; records from lines over 64 KiB also carry `partial: true`. The path is opened in `O_APPEND|O_CREATE|O_WRONLY` mode `0600`. If unset, no live log is written. The full byte-exact stdout/stderr objects are still uploaded to S3 when the command finishes.
