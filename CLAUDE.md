...
- `gorn serve --config X` — HA daemon. Campaigns for leadership; when elected, runs a per-endpoint goroutine pool that dispatches tasks via SSH, and serves `GET /v1/inflight` (guid → host) on `serve.listen` for `control` to read. On leadership loss: `os.Exit(0)` (let systemd restart).
- `gorn control --config X` — HTTP JSON RPC over etcd + S3. Listens on `control.listen`. Endpoints: `POST /v1/tasks` (enqueue, auto-GUID, stamps `enqueued_at`), `GET /v1/tasks` (queue list), `GET /v1/tasks/<guid>` (state: `queued|done|not_found`), `GET /v1/tasks/<guid>/output` (parsed `result.json` + base64 stdout/stderr), `GET /v1/endpoints` (endpoint list from config). No leader election; enqueue is an etcd `CreateRevision==0` txn, any instance can serve. `GET /v1/tasks` stamps `host` on running tasks by resolving the leader from the etcd election key and calling its `/v1/inflight`; that call failing degrades to no `host`, never to a failed listing.
- `gorn web --config X` — dashboard with no external assets (CSS is inline, no CDN; light and dark both styled). Reads only `web.api` and `web.listen` from the config; talks to `control` via HTTP. `/` is one table of queued *and* running tasks — running first, marked by a non-empty `host` — and `/endpoints` is the workers; each page fetches only its own API. The queue page polls `/api/tasks` (a proxy for `control`, which usually listens on loopback and is unreachable from the browser) every 2s and patches the DOM instead of reloading, and ticks ages locally each second. Read-only, never goes to etcd/S3 directly.
