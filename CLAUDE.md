...
- `gorn web --config X` — Bootstrap-CSS dashboard. Reads only `web.api` and `web.listen` from the config; talks to `control` via HTTP. Renders the queue with per-task age at `/` and workers at `/endpoints`; each page fetches only its own API. Auto-refreshes every 2s. Read-only, never goes to etcd/S3 directly.
