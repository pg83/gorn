...
Prefer native Go libraries over shelling out to binaries, because error surfaces are richer and more tractable:

- SSH: `golang.org/x/crypto/ssh` (native), not `exec.Command("ssh", ...)`.
